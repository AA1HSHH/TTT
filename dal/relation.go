...
func IsUserExistById(id int64) bool {
	//var userinfo User // 是否要加 {}
	//if err := db.Where("id=?", id).Select("id").First(&User).Error; err != nil {
	//	log.Println(err)
	//}
...
func CancelUserFollow(userId, userToId int64) error {
	...
	//	return err
	//	}
	//	fmt.Println("len(tfollow)",len(tfollow))
	//if len(tfollow) == 0 {
...
func  GetFollowListByUserId(userId int64) ([]User,error){
	var err error
	var results []User
